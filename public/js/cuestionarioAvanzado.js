...
botonSiguiente.onclick = ()=>{
    ...
    if(contadorPreguntas<questionsA.length - 1){
        contadorPreguntas ++;
...
function mostrarIndicaciones(){
    ...
    let indicacion = '<div class="info">1.El cuestionario consta de '+ questionsAnum+ ' preguntas.</div>'
                    + '<div class="info">2.Las preguntas que contiene el cuestionario son de complejidad avanzada.</div>'
...
function mostrarPreguntas(index){
    ...
    titulo.textContent = " "+nombreA;
    const preguntastxt = document.querySelector(".questiontext");
    const numPreguntas = document.querySelector(".numeropreguntas");
    let lineaPregunta = '<span>Pregunta ' + questionsA[index].num+ '. '+questionsA[index].question + '</span>';
    let lineaOpcion = '<div class="option"> <span>'+questionsA[index].options[0]+'</span></div>'
                    + '<div class="option"> <span>'+questionsA[index].options[1]+'</span></div>';
    let lineaOpcion1 = '<div class="option"> <span>'+questionsA[index].options[2]+'</span></div>'
    ...
    let contadorPreguntastxt = '<span><p>Pregunta '+num+' de '+questionsA.length+'</p></span>';
    numPreguntas.innerHTML = contadorPreguntastxt;
    let puntajeFootertxt = '<span><p>Puntos:'+puntaje+'/'+questionsA.length+'</p></span>';
...
function optionSelected(answer){
    ...
    let correctAns = " "+ questionsA[contadorPreguntas].answer;
...
function contarTiempo(time){
    ...
    function timer(){
        ...
        if(time < 0){ 
            ...
            let correcAns = " "+questionsA[contadorPreguntas].answer; 
...
function mostrarResultado(){
    ...
    let nota = (puntaje/questionsA.length)*10;
    if(nota>8){
        puntajetxt.innerHTML = '<span><p>Felicidades aprobaste el cuestionario, obtuviste '+puntaje+' de '+questionsA.length+' puntos </p></span>'
                            +   '<span>Tu nota es de '+nota.toFixed(2)+'</span>';
    }else{
        if(nota>5){
            puntajetxt.innerHTML = '<span><p>Sigue esforzandote aún puedes mejorar, obtuviste '+puntaje+' de '+questionsA.length+' puntos </p></span>'
                               +   '<span>Tu nota es de '+nota.toFixed(2)+'</span>';
        }else{
            puntajetxt.innerHTML = '<span><p>Lo siento reprobaste el cuestionario, obtuviste '+puntaje+' de '+questionsA.length+' puntos </p></span>'
                                +   '<span>Tu nota es de '+nota.toFixed(2)+'</span>';
        }
    }
